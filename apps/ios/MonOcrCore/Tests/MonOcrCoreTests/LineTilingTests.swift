...
/**
 ...
 `python/monocr_onnx/segmenter.py`. Tiling was measured against squeezing on that
 implementation, so a port that cuts at different columns is not the thing the
 measurement applies to — which is why these tests pin cut positions and not just
 tile counts.
 ...
 not reproduce. The live measurement is `mon_OCR/eval/tiling-ab-2026-08-22.md`
